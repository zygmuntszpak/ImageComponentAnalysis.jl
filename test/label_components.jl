@testset "label components 2D" begin
   ...
   for T in (Int, Bool, Gray{Bool}, Gray{N0f8}, Gray{N0f16}, Gray{N0f32}, Gray{Float64})
        for i = 1:20
            ...
            labels = ImageComponentAnalysis.label_components(test_image, trues(3,3))
...
@testset "label components 3D" begin
    for T in (Int, Bool, Gray{Bool}, Gray{N0f8}, Gray{N0f16}, Gray{N0f32}, Gray{Float64})
         for i = 1:20
             test_volume = eval(Symbol("test_volume_$(i)"))
             labels = ImageComponentAnalysis.label_components(test_volume, trues(3,3,3))
             num_components = maximum(labels)
